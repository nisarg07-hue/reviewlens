...
export async function POST(req: Request) {
  try {
    ...
    const { variantId: rawVariantId } = await req.json();

    if (!rawVariantId) {
      return NextResponse.json({ error: "variantId is required" }, { status: 400 });
    }

    const storeId = Number(process.env.NEXT_PUBLIC_LEMONSQUEEZY_STORE_ID);
    ...
    const planString = variantId === 1607145 ? "pro" : "agency";
